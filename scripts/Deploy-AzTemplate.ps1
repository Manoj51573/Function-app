...
if ($isBicep){
    Write-Host "Creating json from bicep" -ForegroundColor DarkBlue
    bicep build $TemplateFile --outdir $ArtifactStagingDirectory
    # now point the deployment to the json file that was just built
...
Write-Host "$TemplateFile found?: $(Test-Path $TemplateFile)"
Write-Host "$TemplateParametersFile found?: $(Test-Path $TemplateParametersFile)"
