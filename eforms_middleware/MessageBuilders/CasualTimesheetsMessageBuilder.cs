...
{
    internal class CasualTimesheetsMessageBuilder : MessageBuilder
    {
        ...
        public CasualTimesheetsMessageBuilder(IConfiguration configuration
        ...
        private async Task<List<MailMessage>> GetSubmittedEmailAsync(FormPermission owner
                                                ...
                                                , CasualTimsheetsModel responseData)
        {
            var subject = $"Reminder: Casual Timesheets eForm #{DbModel.FormInfoId} requires your approval";
            var body = string.Format(CasualTimesheetsTemplate.ReminderToApprover
                             , owner.Name
                             , DbModel.FormInfoId
        ...
        private async Task<List<MailMessage>> GetDelegatedMail(FormPermission owner
                                       , List<FormPermission> currentApprovers)
        {
            var subject = $"Reminder: Casual Timesheets eForm #{DbModel.FormInfoId} has been delegated for your approval";

            var body = string.Format(CasualTimesheetsTemplate.Delegated
                            , owner.Name
                            , DbModel.FormInfoId
                            , $"{SummaryUrl}{DbModel.FormInfoId}");
            //fix the below dpf
        ...
        private async Task<List<MailMessage>> GetEscalatedMail(FormPermission owner
                                        , List<FormPermission> currentApprovers)
        {
            var subject = $"Reminder: Casual Timesheets eForm #{DbModel.FormInfoId} has escalated for your review";
            var body = string.Format(CasualTimesheetsTemplate.Escalated
                  , owner.Name
                            , DbModel.FormInfoId
                            , $"{SummaryUrl}{DbModel.FormInfoId}");
            //fix the below dpf
        ...
        private async Task<List<MailMessage>> GetEscalationNotificationToPODeFormsBusinessAdminsGroup(FormPermission owner
                                           , List<FormPermission> currentApprovers)
        {
            var subject = $"Casual Timesheets eForm #{DbModel.FormInfoId} has escalated for your action";
            var body = string.Format(CasualTimesheetsTemplate.EscalationNotificationToPODeFormsBusinessAdminsGroup
                 , owner.Name
                 , DbModel.FormInfoId
        ...
        protected override async Task<List<MailMessage>> GetMessageInternalAsync()
        {
            ...
            try
            {
                ...
                var responseData = JsonConvert.DeserializeObject<CasualTimsheetsModel>(DbModel.Response);
