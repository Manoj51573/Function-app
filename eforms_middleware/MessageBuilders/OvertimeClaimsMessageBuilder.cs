...
{
    internal class OvertimeClaimsMessageBuilder : MessageBuilder
    {
        ...
        public OvertimeClaimsMessageBuilder(IConfiguration configuration
        ...
        private async Task<List<MailMessage>> GetSubmittedEmailAsync(FormPermission owner
                                                ...
                                                , OvertimeClaimsModel responseData)
        {
            var subject = $"Reminder: Overtime Claims eForm #{DbModel.FormInfoId} requires your approval";
            var body = string.Format(OvertimeClaimsTemplate.ReminderToApprover
                             , owner.Name
                             , DbModel.FormInfoId
        ...
        private async Task<List<MailMessage>> GetDelegatedMail(FormPermission owner
                                       , List<FormPermission> currentApprovers)
        {
            var subject = $"Reminder: Overtime Claims eForm #{DbModel.FormInfoId} has been delegated for your approval";

            var body = string.Format(OvertimeClaimsTemplate.Delegated
                            , owner.Name
                            , DbModel.FormInfoId
                            , $"{SummaryUrl}{DbModel.FormInfoId}");
            //fix the below dpf
        ...
        private async Task<List<MailMessage>> GetEscalatedMail(FormPermission owner
                                        , List<FormPermission> currentApprovers)
        {
            var subject = $"Reminder: Overtime Claims eForm #{DbModel.FormInfoId} has escalated for your review";
            var body = string.Format(OvertimeClaimsTemplate.Escalated
                  , owner.Name
                            , DbModel.FormInfoId
                            , $"{SummaryUrl}{DbModel.FormInfoId}");
            //fix the below dpf
        ...
        private async Task<List<MailMessage>> GetEscalationNotificationToPODeFormsBusinessAdminsGroup(FormPermission owner
                                           , List<FormPermission> currentApprovers)
        {
            var subject = $"Overtime Claims eForm #{DbModel.FormInfoId} has escalated for your action";
            var body = string.Format(OvertimeClaimsTemplate.EscalationNotificationToPODeFormsBusinessAdminsGroup
                 , owner.Name
                 , DbModel.FormInfoId
        ...
        protected override async Task<List<MailMessage>> GetMessageInternalAsync()
        {
            ...
            try
            {
                ...
                var responseData = JsonConvert.DeserializeObject<OvertimeClaimsModel>(DbModel.Response);
