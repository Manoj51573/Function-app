...
{
    internal class SeaGoingAllowanceClaimsMessageBuilder : MessageBuilder
    {
        ...
        public SeaGoingAllowanceClaimsMessageBuilder(IConfiguration configuration
        ...
        private async Task<List<MailMessage>> GetSubmittedEmailAsync(FormPermission owner
                                                ...
                                                , SeaGoingAllowanceClaimModel responseData)
        {
            var subject = $"Reminder: Sea Going Allowance Claim eForm #{DbModel.FormInfoId} requires your approval";
            var body = string.Format(SeaGoingAllowanceClaimsTemplate.ReminderToApprover
                             , owner.Name
                             , DbModel.FormInfoId
        ...
        private async Task<List<MailMessage>> GetDelegatedMail(FormPermission owner
                                       , List<FormPermission> currentApprovers)
        {
            var subject = $"Reminder: Sea Going Allowance Claim eForm #{DbModel.FormInfoId} has been delegated for your approval";

            var body = string.Format(SeaGoingAllowanceClaimsTemplate.Delegated
                            , owner.Name
                            , DbModel.FormInfoId
                            , $"{SummaryUrl}{DbModel.FormInfoId}");
            //fix the below dpf
        ...
        private async Task<List<MailMessage>> GetEscalatedMail(FormPermission owner
                                        , List<FormPermission> currentApprovers)
        {
            var subject = $"Reminder: Sea Going Allowance Claim eForm #{DbModel.FormInfoId} has escalated for your review";
            var body = string.Format(SeaGoingAllowanceClaimsTemplate.Escalated
                  , owner.Name
                            , DbModel.FormInfoId
                            , $"{SummaryUrl}{DbModel.FormInfoId}");
            //fix the below dpf
        ...
        private async Task<List<MailMessage>> GetEscalationNotificationToPODeFormsBusinessAdminsGroup(FormPermission owner
                                           , List<FormPermission> currentApprovers)
        {
            var subject = $"Sea Going Allowance Claim eForm #{DbModel.FormInfoId} has escalated for your action";
            var body = string.Format(SeaGoingAllowanceClaimsTemplate.EscalationNotificationToPODeFormsBusinessAdminsGroup
                 , owner.Name
                 , DbModel.FormInfoId
        ...
        protected override async Task<List<MailMessage>> GetMessageInternalAsync()
        {
            ...
            try
            {
                ...
                var responseData = JsonConvert.DeserializeObject<SeaGoingAllowanceClaimModel>(DbModel.Response);
