...
{
    internal class PenaltyShiftAllowanceClaimsMessageBuilder : MessageBuilder
    {
        ...
        public PenaltyShiftAllowanceClaimsMessageBuilder(IConfiguration configuration
        ...
        private async Task<List<MailMessage>> GetSubmittedEmailAsync(FormPermission owner
                                                ...
                                                , PenaltyShiftAllowanceClaimsModel responseData)
        {
            var subject = $"Reminder: Penalty Shift Allowance Claims eForm #{DbModel.FormInfoId} requires your approval";
            var body = string.Format(PenaltyShiftAllowanceClaimsTemplate.ReminderToApprover
                             , owner.Name
                             , DbModel.FormInfoId
        ...
        private async Task<List<MailMessage>> GetDelegatedMail(FormPermission owner
                                       , List<FormPermission> currentApprovers)
        {
            var subject = $"Reminder: Penalty Shift Allowance Claims eForm #{DbModel.FormInfoId} has been delegated for your approval";

            var body = string.Format(PenaltyShiftAllowanceClaimsTemplate.Delegated
                            , owner.Name
                            , DbModel.FormInfoId
                            , $"{SummaryUrl}{DbModel.FormInfoId}");
            //fix the below dpf
        ...
        private async Task<List<MailMessage>> GetEscalatedMail(FormPermission owner
                                        , List<FormPermission> currentApprovers)
        {
            var subject = $"Reminder: Penalty Shift Allowance Claims eForm #{DbModel.FormInfoId} has escalated for your review";
            var body = string.Format(PenaltyShiftAllowanceClaimsTemplate.Escalated
                  , owner.Name
                            , DbModel.FormInfoId
                            , $"{SummaryUrl}{DbModel.FormInfoId}");
            //fix the below dpf
        ...
        private async Task<List<MailMessage>> GetEscalationNotificationToPODeFormsBusinessAdminsGroup(FormPermission owner
                                           , List<FormPermission> currentApprovers)
        {
            var subject = $"Penalty Shift Allowance Claims eForm #{DbModel.FormInfoId} has escalated for your action";
            var body = string.Format(PenaltyShiftAllowanceClaimsTemplate.EscalationNotificationToPODeFormsBusinessAdminsGroup
                 , owner.Name
                 , DbModel.FormInfoId
        ...
        protected override async Task<List<MailMessage>> GetMessageInternalAsync()
        {
            ...
            try
            {
                ...
                var responseData = JsonConvert.DeserializeObject<PenaltyShiftAllowanceClaimsModel>(DbModel.Response);
