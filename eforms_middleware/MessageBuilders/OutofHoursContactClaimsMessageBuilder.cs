...
{
    internal class OutofHoursContactClaimsMessageBuilder : MessageBuilder
    {
        ...
        public OutofHoursContactClaimsMessageBuilder(IConfiguration configuration
        ...
        private async Task<List<MailMessage>> GetSubmittedEmailAsync(FormPermission owner
                                                ...
                                                , OutofHoursContactClaimsModel responseData)
        {
            var subject = $"Reminder: Out of Hours Contact Claims eForm #{DbModel.FormInfoId} requires your approval";
            var body = string.Format(OutofHoursContactClaimsTemplate.ReminderToApprover
                             , owner.Name
                             , DbModel.FormInfoId
        ...
        private async Task<List<MailMessage>> GetDelegatedMail(FormPermission owner
                                       , List<FormPermission> currentApprovers)
        {
            var subject = $"Reminder: Out of Hours Contact Claims eForm #{DbModel.FormInfoId} has been delegated for your approval";

            var body = string.Format(OutofHoursContactClaimsTemplate.Delegated
                            , owner.Name
                            , DbModel.FormInfoId
                            , $"{SummaryUrl}{DbModel.FormInfoId}");
            //fix the below dpf
        ...
        private async Task<List<MailMessage>> GetEscalatedMail(FormPermission owner
                                        , List<FormPermission> currentApprovers)
        {
            var subject = $"Reminder: Out of Hours Contact Claims eForm #{DbModel.FormInfoId} has escalated for your review";
            var body = string.Format(OutofHoursContactClaimsTemplate.Escalated
                  , owner.Name
                            , DbModel.FormInfoId
                            , $"{SummaryUrl}{DbModel.FormInfoId}");
            //fix the below dpf
        ...
        private async Task<List<MailMessage>> GetEscalationNotificationToPODeFormsBusinessAdminsGroup(FormPermission owner
                                           , List<FormPermission> currentApprovers)
        {
            var subject = $"Out of Hours Contact Claims eForm #{DbModel.FormInfoId} has escalated for your action";
            var body = string.Format(OutofHoursContactClaimsTemplate.EscalationNotificationToPODeFormsBusinessAdminsGroup
                 , owner.Name
                 , DbModel.FormInfoId
        ...
        protected override async Task<List<MailMessage>> GetMessageInternalAsync()
        {
            ...
            try
            {
                ...
                var responseData = JsonConvert.DeserializeObject<OutofHoursContactClaimsModel>(DbModel.Response);
