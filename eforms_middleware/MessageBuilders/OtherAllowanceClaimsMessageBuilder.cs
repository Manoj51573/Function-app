...
{
    internal class OtherAllowanceClaimsMessageBuilder : MessageBuilder
    {
        ...
        public OtherAllowanceClaimsMessageBuilder(IConfiguration configuration
        ...
        private async Task<List<MailMessage>> GetSubmittedEmailAsync(FormPermission owner
                                                ...
                                                , OtherAllowanceClaimsModel responseData)
        {
            var subject = $"Reminder: Other Allowance Claim eForm #{DbModel.FormInfoId} requires your approval";
            var body = string.Format(OtherAllowanceClaimsTemplate.ReminderToApprover
                             , owner.Name
                             , DbModel.FormInfoId
        ...
        private async Task<List<MailMessage>> GetDelegatedMail(FormPermission owner
                                       , List<FormPermission> currentApprovers)
        {
            var subject = $"Reminder: Other Allowance Claim eForm #{DbModel.FormInfoId} has been delegated for your approval";

            var body = string.Format(OtherAllowanceClaimsTemplate.Delegated
                            , owner.Name
                            , DbModel.FormInfoId
                            , $"{SummaryUrl}{DbModel.FormInfoId}");
            //fix the below dpf
        ...
        private async Task<List<MailMessage>> GetEscalatedMail(FormPermission owner
                                        , List<FormPermission> currentApprovers)
        {
            var subject = $"Reminder: Other Allowance Claim eForm #{DbModel.FormInfoId} has escalated for your review";
            var body = string.Format(OtherAllowanceClaimsTemplate.Escalated
                  , owner.Name
                            , DbModel.FormInfoId
                            , $"{SummaryUrl}{DbModel.FormInfoId}");
            //fix the below dpf
        ...
        private async Task<List<MailMessage>> GetEscalationNotificationToPODeFormsBusinessAdminsGroup(FormPermission owner
                                           , List<FormPermission> currentApprovers)
        {
            var subject = $"Other Allowance Claim eForm #{DbModel.FormInfoId} has escalated for your action";
            var body = string.Format(OtherAllowanceClaimsTemplate.EscalationNotificationToPODeFormsBusinessAdminsGroup
                 , owner.Name
                 , DbModel.FormInfoId
        ...
        protected override async Task<List<MailMessage>> GetMessageInternalAsync()
        {
            ...
            try
            {
                ...
                var responseData = JsonConvert.DeserializeObject<OtherAllowanceClaimsModel>(DbModel.Response);
