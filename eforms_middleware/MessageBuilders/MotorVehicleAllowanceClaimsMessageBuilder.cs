...
{
    internal class MotorVehicleAllowanceClaimsMessageBuilder : MessageBuilder
    {
        ...
        public MotorVehicleAllowanceClaimsMessageBuilder(IConfiguration configuration
        ...
        private async Task<List<MailMessage>> GetSubmittedEmailAsync(FormPermission owner
                                                ...
                                                , MotorVehicleAllowanceClaimsModel responseData)
        {
            var subject = $"Reminder: Motor Vehicle Allowance Claims eForm #{DbModel.FormInfoId} requires your approval";
            var body = string.Format(MotorVehicleAllowanceClaimsTemplate.ReminderToApprover
                             , owner.Name
                             , DbModel.FormInfoId
        ...
        private async Task<List<MailMessage>> GetDelegatedMail(FormPermission owner
                                       , List<FormPermission> currentApprovers)
        {
            var subject = $"Reminder: Motor Vehicle Allowance Claims eForm #{DbModel.FormInfoId} has been delegated for your approval";

            var body = string.Format(MotorVehicleAllowanceClaimsTemplate.Delegated
                            , owner.Name
                            , DbModel.FormInfoId
                            , $"{SummaryUrl}{DbModel.FormInfoId}");
            //fix the below dpf
        ...
        private async Task<List<MailMessage>> GetEscalatedMail(FormPermission owner
                                        , List<FormPermission> currentApprovers)
        {
            var subject = $"Reminder: Motor Vehicle Allowance Claims eForm #{DbModel.FormInfoId} has escalated for your review";
            var body = string.Format(MotorVehicleAllowanceClaimsTemplate.Escalated
                  , owner.Name
                            , DbModel.FormInfoId
                            , $"{SummaryUrl}{DbModel.FormInfoId}");
            //fix the below dpf
        ...
        private async Task<List<MailMessage>> GetEscalationNotificationToPODeFormsBusinessAdminsGroup(FormPermission owner
                                           , List<FormPermission> currentApprovers)
        {
            var subject = $"Motor Vehicle Allowance Claims eForm #{DbModel.FormInfoId} has escalated for your action";
            var body = string.Format(MotorVehicleAllowanceClaimsTemplate.EscalationNotificationToPODeFormsBusinessAdminsGroup
                 , owner.Name
                 , DbModel.FormInfoId
        ...
        protected override async Task<List<MailMessage>> GetMessageInternalAsync()
        {
            ...
            try
            {
                ...
                var responseData = JsonConvert.DeserializeObject<MotorVehicleAllowanceClaimsModel>(DbModel.Response);
