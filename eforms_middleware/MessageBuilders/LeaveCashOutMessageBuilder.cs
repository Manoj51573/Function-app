...
public class LeaveCashOutMessageBuilder : LCOMessageBuilder
{
    private readonly ILogger<LeaveCashOutMessageBuilder> _logger;
    protected override string EditPath => "leave-cash-out";
    protected override string SummaryPath => "leave-cash-out/summary";
    protected override string FormTypeSubject => "Leave Cash Out";
    protected override string FormTypeBody => "";
    public LeaveCashOutMessageBuilder(IEmployeeService employeeService, IRepository<TaskInfo> taskInfoRepo,
        ILogger<LeaveCashOutMessageBuilder> logger, IConfiguration configuration, IRepository<FormPermission> formPermissionRepo,
    ...
    protected async Task<List<MailMessage>> GetCompletedEmail(bool ccManager = true)
    {
        ...
        if (leaveModel.IsLeaveRequesterIsManager == "Yes")
        {
            ...
            {
                var ownerMessageBody = string.Format(LeaveAmendmentTemplates.COMPLETED_TEMPLATE_LEAVE_CASH_OUT
                    , formOwnerUser.EmployeePreferredFullName, FormTypeSubject, DbModel.FormInfoId, SummaryHref, SuffixText
                    , leaveModel.LeaveApplicationType, leaveModel.HoursToCashOut, leaveModel.GrossPaymentValue
                    , leaveModel.TaxWithheldValue, leaveModel.NetPaymentValue, leaveModel.PaymentDateValue, leaveModel.ExcisedbyCalendarDayvalue);
        ...
        else
        {
            ...
            var body = string.Format(LeaveAmendmentTemplates.COMPLETED_TEMPLATE_LEAVE_CASH_OUT, DbModel.FormOwnerName, FormTypeSubject
