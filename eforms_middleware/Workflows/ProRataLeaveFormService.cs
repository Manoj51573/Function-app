...
{
    public class ProRataLeaveFormService: BaseApprovalService
    {
        private readonly ILogger<ProRataLeaveFormService> _logger;
        ...
        public ProRataLeaveFormService(IFormEmailService formEmailService, IRepository<AdfGroup> adfGroup,
            ...
            IRequestingUserProvider requestingUserProvider, IEmployeeService employeeService, ILogger<ProRataLeaveFormService> logger,
        ...
        public async Task<RequestResult> ProRataLeaveApprovalProcess(LeaveInfoInsertModel formInfoInsertModel)
        {
            await SetupBaseApprovalService(formInfoInsertModel.FormDetails.FormInfoID);
            var permissions = new List<FormPermission>();
            var proRataModel = JsonConvert.DeserializeObject<ProRataModel>(formInfoInsertModel.FormDetails.Response);
            ...
            proRataModel.ReasonForDecision = formInfoInsertModel.RejectionReason;
            ...
            switch (formStatus)
            {
                case FormStatus.Unsubmitted:
                    dbForm.Response = JsonConvert.SerializeObject(proRataModel);
                    dbForm.FormApprovers = "";
                    if (proRataModel.IsLeaveRequesterIsManager == "Yes"){
                ...
                case FormStatus.Rejected:
                    var originalResponse = JsonConvert.DeserializeObject<ProRataModel>(dbForm.Response);
                    originalResponse.ReasonForDecision = proRataModel.ReasonForDecision;
                ...
                case FormStatus.Submitted:
                    proRataModel.ReasonForDecision = string.Empty;
                    dbForm.Response = JsonConvert.SerializeObject(proRataModel);
                    ...
                    else if (RequestingUser.Managers.Any() || RequestingUser.ExecutiveDirectors.Any())
                    {
                        if (proRataModel.IsLeaveRequesterIsManager == "Yes")
                        {
                            ...
                            var onBehalfOf = await EmployeeService.GetEmployeeByAzureIdAsync(proRataModel.allEmployees[0].ActiveDirectoryId);
                            permissions.Add(new FormPermission((byte)PermissionFlag.View, userId: proRataModel.allEmployees[0].ActiveDirectoryId));
                ...
                case FormStatus.Approved:
                    ...
                    else
                    {
                        ...
                        if(dbForm.NextApprover== FormOwner.ManagerIdentifier || FormOwner.ManagerIdentifier==null)
                        {
                            dbForm.NextApprover = group.GroupName; //await GetPodEformsBusinessAdminGroup();
                            permissions.Add(new FormPermission((byte)PermissionFlag.UserActionable, groupId: LeaveForms.POD_ESO_GROUP_ID));
                            dbForm.NextApprovalLevel = group.GroupName;
                        ...
                        {
                            if (FormOwner.EmployeeManagementTier is 1 || FormOwner.EmployeeManagementTier is 2)
                            {
                                dbForm.NextApprover = group.GroupName; //await GetPodEformsBusinessAdminGroup();
                                permissions.Add(new FormPermission((byte)PermissionFlag.UserActionable, groupId: LeaveForms.POD_ESO_GROUP_ID));
                                dbForm.NextApprovalLevel = group.GroupName;
                            ...
                            else if (FormOwner.Managers.Any())
                            {
                            ...
                            else if (FormOwner.ExecutiveDirectors.Any())
                            {
                ...
                case FormStatus.IndependentReview:
                    ...
                    statusBtnData.StatusBtnModel = new List<StatusBtnModel>
                {
                    SetStatusBtnData(FormStatus.Submitted, FormStatus.Approve.ToString(), true),
                    SetStatusBtnData(FormStatus.Cancelled, FormStatus.Cancel.ToString(), true),
                    SetStatusBtnData(FormStatus.Unsubmitted, FormStatus.Reject.ToString(),false,FormStatus.Rejected.ToString() )
                };
                    break;
            ...
            try
            {
                ...
                if (invalidRequest is "")
                {
                    ...
                    await UpdateTasksAsyncLeaveCashOut(formInfo, FormType.LcR_PLS, formInfoInsertModel.FormDetails.FormStatusID);
                    await _permissionManager.UpdateFormPermissionsAsync(formInfo.FormInfoId, permissions);
                     await UpdateHistoryAsync(request, formInfo, ActioningGroup?.Id, formInfoInsertModel.AdditionalInfo, proRataModel.ReasonForDecision);
