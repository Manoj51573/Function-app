...
{
    public class PurchaseLeaveFormService:BaseApprovalService
    {
        private readonly ILogger<PurchaseLeaveFormService> _logger;
        ...
        public PurchaseLeaveFormService(IFormEmailService formEmailService, IRepository<AdfGroup> adfGroup,
            ...
            IRequestingUserProvider requestingUserProvider, IEmployeeService employeeService, ILogger<PurchaseLeaveFormService> logger,
        ...
        public async Task<RequestResult> PurchasedLeaveApprovalProcess(LeaveInfoInsertModel formInfoInsertModel)
        {
            await SetupBaseApprovalService(formInfoInsertModel.FormDetails.FormInfoID);
            var permissions = new List<FormPermission>();
            var purchasedLeaveModel = JsonConvert.DeserializeObject<PurchasedLeaveModel>(formInfoInsertModel.FormDetails.Response);
            ...
            purchasedLeaveModel.ReasonForDecision = formInfoInsertModel.RejectionReason;
            ...
            switch (formStatus)
            {
                case FormStatus.Unsubmitted:
                    dbForm.Response = JsonConvert.SerializeObject(purchasedLeaveModel);
                    dbForm.FormApprovers = "";
                    if (purchasedLeaveModel.IsLeaveRequesterIsManager == "Yes"){
                ...
                case FormStatus.Rejected:
                    var originalResponse = JsonConvert.DeserializeObject<PurchasedLeaveModel>(dbForm.Response);
                    originalResponse.ReasonForDecision = purchasedLeaveModel.ReasonForDecision;
                ...
                case FormStatus.Submitted:
                    ...
                    purchasedLeaveModel.ReasonForDecision = string.Empty;
                    dbForm.Response = JsonConvert.SerializeObject(purchasedLeaveModel);
                    ...
                    else if (RequestingUser.Managers.Any() || RequestingUser.ExecutiveDirectors.Any())
                    {
                        if (purchasedLeaveModel.IsLeaveRequesterIsManager == "Yes")
                        {
                            ...
                            var onBehalfOf = await EmployeeService.GetEmployeeByAzureIdAsync(purchasedLeaveModel.allEmployees[0].ActiveDirectoryId);
                            permissions.Add(new FormPermission((byte)PermissionFlag.View, userId: purchasedLeaveModel.allEmployees[0].ActiveDirectoryId));
            ...
            try
            {
                ...
                if (invalidRequest is "")
                {
                    ...
                    await UpdateTasksAsyncLeaveCashOut(formInfo,purchasedLeaveModel.LeaveApplicationType== "Purchased Leave"?FormType.LcR_PLA: FormType.LcR_DSA, formInfoInsertModel.FormDetails.FormStatusID);
                    await _permissionManager.UpdateFormPermissionsAsync(formInfo.FormInfoId, permissions);
                     await UpdateHistoryAsync(request, formInfo, ActioningGroup?.Id, formInfoInsertModel.AdditionalInfo, purchasedLeaveModel.ReasonForDecision);
