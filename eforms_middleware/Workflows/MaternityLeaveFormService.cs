...
public class MaternityLeaveFormService : BaseApprovalService
{
    private readonly ILogger<MaternityLeaveFormService> _logger;
    ...
    public MaternityLeaveFormService(IFormEmailService formEmailService, IRepository<AdfGroup> adfGroup,
        ...
        IRequestingUserProvider requestingUserProvider, IEmployeeService employeeService, ILogger<MaternityLeaveFormService> logger,
    ...
    public async Task<RequestResult> MaternityLeaveApprovalProcess(LeaveInfoInsertModel formInfoInsertModel)
    {
        try
        {
        await SetupBaseApprovalService(formInfoInsertModel.FormDetails.FormInfoID);
        var permissions = new List<FormPermission>();
        var maternityLeaveModel = JsonConvert.DeserializeObject<MaternityLeaveModel>(formInfoInsertModel.FormDetails.Response);
        ...
        maternityLeaveModel.ReasonForDecision = formInfoInsertModel.RejectionReason;
        ...
        switch (formStatus)
        {
            case FormStatus.Unsubmitted:
                dbForm.Response = JsonConvert.SerializeObject(maternityLeaveModel);
                dbForm.FormApprovers = "";
                if (maternityLeaveModel.isLeaveRequesterIsManager == "Yes")
                {
            ...
            case FormStatus.Rejected:
                var originalResponse = JsonConvert.DeserializeObject<MaternityLeaveModel>(dbForm.Response);
                originalResponse.ReasonForDecision = maternityLeaveModel.ReasonForDecision;
            ...
            case FormStatus.Submitted:
                ...
                var group = await GetAdfGroupById(IsRequestorDVS ? LeaveForms.DOT_DVS_FIXED_TERM_APPOINTMENT_QA_GROUP_ID : LeaveForms.POD_ESO_GROUP_ID);
                var NextApproverGroupId = IsRequestorDVS ? LeaveForms.DOT_DVS_FIXED_TERM_APPOINTMENT_QA_GROUP_ID : LeaveForms.POD_ESO_GROUP_ID;

                maternityLeaveModel.ReasonForDecision = string.Empty;
                dbForm.Response = JsonConvert.SerializeObject(maternityLeaveModel);
                ...
                {
                    if (maternityLeaveModel.isLeaveRequesterIsManager is "Yes")
                    {
                        var onBehalfOf = await EmployeeService.GetEmployeeByAzureIdAsync(maternityLeaveModel.allEmployees[0].ActiveDirectoryId);
                        ...
                        else
                        {
                            ...
                            permissions.Add(new FormPermission((byte)PermissionFlag.View, userId: maternityLeaveModel.allEmployees[0].ActiveDirectoryId));
            ...
            case FormStatus.Approved:
                ...
                if (IsRequestorDVS && dbForm.NextApprover != LeaveForms.DOT_EMPLOYEE_SERVICES_OFFICER_GROUP && dbForm.NextApprover != LeaveForms.DOT_DVS_FIXED_TERM_APPOINTMENT_QA_GROUP_NAME)
                {
                ...
                {
                    dbForm.NextApprover = LeaveForms.DOT_EMPLOYEE_SERVICES_OFFICER_GROUP;
                    permissions.Add(new FormPermission((byte)PermissionFlag.UserActionable, groupId: LeaveForms.POD_ESO_GROUP_ID));
                    dbForm.NextApprovalLevel = LeaveForms.DOT_EMPLOYEE_SERVICES_OFFICER_GROUP;
                    statusBtnData.StatusBtnModel = new List<StatusBtnModel>
                    {
                        SetStatusBtnData(FormStatus.Completed, FormStatus.Complete.ToString(), true),
                        SetStatusBtnData(FormStatus.Cancelled, FormStatus.Cancel.ToString(), true),
            ...
            if (invalidRequest is "")
            {
                ...
                await UpdateTasksAsyncLeaveCashOut(formInfo, FormType.LcR_MA, formInfoInsertModel.FormDetails.FormStatusID);
                await _permissionManager.UpdateFormPermissionsAsync(formInfo.FormInfoId, permissions);
                await UpdateHistoryAsync(request, formInfo, ActioningGroup?.Id, formInfoInsertModel.AdditionalInfo, maternityLeaveModel.ReasonForDecision);
